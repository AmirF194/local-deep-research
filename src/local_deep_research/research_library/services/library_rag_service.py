    ...
    def _migrate_legacy_index_files(
        ...
        with lock:
            ...
            try:
                ...
                # Relocate phase-1's text-free .idmap.json sidecar ALONGSIDE the
                # .faiss. phase-2 rekey looks for the sidecar next to the .faiss
                # (legacy_rekey._sidecar_for = <stem>.idmap.json); leaving it at
                # the legacy location orphans it, and a not-yet-rekeyed index then
                # has no findable sidecar → rekey sees "no sidecar + old format"
                # and destructively quarantines a healthy index. Move it too.
