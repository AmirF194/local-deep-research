...
def _purge_redundant_pkl(faiss_path: Path, db_password: str) -> bool:
    ...
    try:
        ...
        logger.error(
            ...
            f"{redact_secrets(str(exc), db_password)}"
        )
        return False
...
def rekey_user_indexes(username: str, db_password: str) -> Dict[str, int]:
    ...
    with get_user_db_session(username, db_password) as session:
        ...
        # Only re-key CURRENT indexes. A collection can have stale historical
        # RAGIndex rows (is_current=False); re-keying one and failing would run
        # _quarantine_and_reset, which resets state by collection_id and would
        # collaterally wipe the healthy CURRENT index's indexed state (forcing
        # a needless full re-embed). Stale rows are never searched, so skipping
        # them is correct; their (already text-free) sidecars simply linger.
        indexes: List[RAGIndex] = (
            session.query(RAGIndex).filter_by(is_current=True).all()
        )
    ...
    for rag_index in indexes:
            ...
            if faiss_path.exists() and fmt is False:
                ...
                try:
                    ...
                    logger.error(
                        ...
                        f"login: {redact_secrets(str(exc), db_password)}"
                    )
                    all_resolved = False
                continue
        ...
        if not faiss_path.exists():
            ...
            # is nothing to re-key. Drop it so it isn't re-checked forever.
            stats["skipped_no_faiss"] += 1
            try:
                sidecar_path.unlink(missing_ok=True)
            except OSError:
                # Withhold the per-user completion marker (mirror the other
                # failure branches). Otherwise the marker is set with the
                # sidecar still on disk, and if this index's .faiss ever
                # reappears the whole rekey step is permanently skipped for
                # the user — the collection would silently never be rekeyed.
                logger.warning(
        ...
        try:
            ...
            logger.error(
                ...
                f"({faiss_path}); will retry next login: "
                f"{redact_secrets(str(exc), db_password)}"
            )
            all_resolved = False
            continue
        ...
        try:
            pos_to_uuid = json.loads(sidecar_path.read_text())
            ...
            logger.error(
                ...
                f"(NOT quarantining — the index file is untouched): "
                f"{redact_secrets(str(exc), db_password)}"
            )
            all_resolved = False
            continue
        ...
        try:
            ...
            logger.error(
                ...
                f"to retry next login: {redact_secrets(str(exc), db_password)}"
            )
            all_resolved = False
