    ...
    for dirpath, dirs, files in os.walk(str(root), onerror=_onerror):
        ...
        for d in dirs:
            ...
            try:
                ...
                logger.error(
                    ...
                    f"({exc}); a legacy .pkl there may NOT have been removed."
                )
                continue
            if is_link:
                scan_errors.append(str(sub))
        ...
        for name in files:
            ...
            try:
                ...
                logger.error(
                    f"RAG plaintext migration could not stat file {p} ({exc}); "
...
def _extract_map(pkl_path: Path) -> bool:
    ...
    try:
        ...
        logger.warning(
            f"Could not write id-map sidecar for {pkl_path} ({exc}); that "
...
def migrate_legacy_docstores() -> Dict[str, int]:
    ...
    for pkl in found:
        ...
        try:
            ...
            logger.warning(f"RAG cache: could not delete {pkl}: {exc}")
