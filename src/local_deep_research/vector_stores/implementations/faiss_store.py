...
class FaissVectorStore(BaseVectorStore):
    ...
    def _live_ids_unlocked(self) -> List[int]:
        if self._index.ntotal == 0:
            return []
        return vector_to_array(self._index.id_map).astype("int64").tolist()
    ...
    def reconstruct(self, id: int) -> Optional[np.ndarray]:
        ...
        with self._read_guard():
            try:
                return self._index.reconstruct(int(id))
            except RuntimeError:
                return None
    ...
    def _apply_locked(
        ...
        with self._lock:
            ...
            try:
                ...
                if add_ids:
                    ids, vecs = (
                        self._dedup_new(add_ids, add_vectors)
                        if dedup
                        else (add_ids, add_vectors)
                    )
                    if ids:
                        self.add(ids, vecs)
                        added = len(ids)
    ...
    def _record_integrity(self, attempts: int = 3) -> None:
        ...
        for _ in range(attempts):
            try:
                self._integrity_record(self._path)
                return
