...
class TestRealGit:
    """End-to-end via a throwaway repo: color and non-UTF8 robustness."""
    ...
    def test_forced_color_does_not_blank_scan(self, monkeypatch, tmp_path):
        ...
        diff, _ = mod._staged_context()
        assert list(mod.iter_added_emails(diff)) == [("d.md", bad)]

    def test_non_utf8_file_does_not_crash(self, monkeypatch, tmp_path):
        run = self._repo(tmp_path)
        (tmp_path / "latin.txt").write_bytes("caf\xe9 note\n".encode("latin-1"))
        run("add", "latin.txt")
        monkeypatch.chdir(tmp_path)
        diff, _ = mod._staged_context()  # must not raise UnicodeDecodeError
